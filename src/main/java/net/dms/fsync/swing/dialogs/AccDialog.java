...
public class AccDialog extends JenixDialog<FenixAcc> {
	...
	public void edit() {
       ...
       /* LocalVariables lv = new LocalVariables();
		...
		String[] responsables = getPayload().getResponsable() != null
								  ? getPayload().getResponsable().split("-")
								  : new String[0];
		String[] esfuerzos = getPayload().getEsfuerzo() != null
							   ? getPayload().getEsfuerzo().split("-")
							   : new String[0];
		String[] subtiposTareas = getPayload().getSubTipo() != null
									? getPayload().getSubTipo().split("-")
									: new String[0];
		for(int i = 0; i < responsables.length; i++) {
			...
			if(fenixResponsable != null) {
				...
				Double esfuerzo = StringUtils.isNotEmpty(esfuerzos[index])
									? Double.parseDouble(esfuerzos[index])
									: null;
	...
	public JComponent createCentralPanel() {
		...
		jtbResponsables.getColumnModel().getColumn(FenixResponsablesTableModel.Columns.SUBTIPO_TAREA.ordinal())
		  .setCellEditor(new DefaultCellEditor(accSubTypeEditor));
...
*/
    ...
    /*    if (txtIdPeticion.equals(null) || StringUtils.isBlank(txtIdPeticion.getText())) {
		...
		if(txtEsfuerzoCliente.getText().equals(null) || StringUtils.isBlank(txtEsfuerzoCliente.getText()) ||
		   txtEsfuerzoCliente.getText().equals("0.0")) { //validacao esfurzo
