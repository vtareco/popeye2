...
import net.dms.fsync.synchronizer.fenix.entities.enumerations.AccIncurridoRowType;
import net.dms.fsync.synchronizer.fenix.entities.enumerations.AccRowType;
...
public class FenixAccMapper {
	...
	public FenixAcc map(Row row) {
		...
		acc.setIdAcc(row.getCell(AccRowType.ID_ACC.getColPosition()) != null
					   ? new Long(new Double(row.getCell(AccRowType.ID_ACC.getColPosition()).getNumericCellValue()).longValue())
					   : null);
		...
		acc.setFechaPrevistaProyecto(row.getCell(AccRowType.FECHA_PREVISTA_PROYECTO.getColPosition()) != null
									   ? row.getCell(AccRowType.FECHA_PREVISTA_PROYECTO.getColPosition()).getDateCellValue()
									   : null);
		acc.setFechaEntrega(row.getCell(AccRowType.FECHA_ENTREGA.getColPosition()) != null
							  ? row.getCell(AccRowType.FECHA_ENTREGA.getColPosition()).getDateCellValue()
							  : null);
		...
		acc.setPuntosHistoria(row.getCell(AccRowType.PUNTOS_HISTORIA.getColPosition()) != null
								? row.getCell(AccRowType.PUNTOS_HISTORIA.getColPosition()).getStringCellValue()
								: null);
		acc.setHistoriaUsuario(row.getCell(AccRowType.HISTORIA_USUARIO.getColPosition()) != null
								 ? row.getCell(AccRowType.HISTORIA_USUARIO.getColPosition()).getStringCellValue()
								 : null);
		acc.setEpica(row.getCell(AccRowType.EPICA.getColPosition()) != null
					   ? row.getCell(AccRowType.EPICA.getColPosition()).getStringCellValue()
					   : null);
		acc.setIdPeticion(row.getCell(AccRowType.ID_PETICION.getColPosition()) != null
							? row.getCell(AccRowType.ID_PETICION.getColPosition()).getStringCellValue()
							: null);
		return acc;
	}
...
// TODO FIXME only for agile
	...
	public FenixAcc mapJiraIssue2Acc(JiraIssue issue) {
		...
		fenixAcc.setDescripcion(StringUtils.truncate(StringUtils.isBlank(issue.getFields().getDescription())
													   ? issue.getFields().getSummary()
													   : issue.getFields().getDescription(), 1024));
		...
		// TODO FIXME AGILE
		fenixAcc.setHistoriaUsuario(issue.getFields().getParent() != null
									  ? issue.getFields().getParent().getKey()
									  : issue.getKey());
		...
		fenixAcc.setResponsable(issue.getFields().getAssignee() != null
								  ? config.searchEverisEmployIdByJiraId(issue.getFields().getAssignee().getKey())
								  : null);
		return fenixAcc;
	}
}
