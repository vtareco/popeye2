...
public class FenixDudaMapper {
	...
	public FenixDuda map(Row row) {
		...
		duda.setFechaAlta(
		  row.getCell(DudaRowType.FECHA_ALTA.getColPosition()) != null && row.getCell(DudaRowType.FECHA_ALTA.getColPosition()).getCellType() != 1
			? row.getCell(DudaRowType.FECHA_ALTA.getColPosition()).getDateCellValue()
			: null);
		duda.setFechaPrevistaRespuesta(row.getCell(DudaRowType.FECHA_PREVISTA_RESPUESTA.getColPosition()) != null
										 ? row.getCell(DudaRowType.FECHA_PREVISTA_RESPUESTA.getColPosition()).getDateCellValue()
										 : null);
